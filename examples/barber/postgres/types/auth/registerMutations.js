...
simfinity.registerMutation(
  ...
  async (input, session) => {
    ...
    const user = (await UserModel.find({ email: { operator: 'EQ', value: input.email.toLowerCase() } }, { session }))[0];
...
simfinity.registerMutation(
  ...
  async (input, session) => {
    ...
    const existing = (await UserModel.find({ email: { operator: 'EQ', value: email } }, { session }))[0];
    ...
    const doc = await UserModel.create({
      ...
      if (error.getCode?.() === 'DUPLICATE_KEY') throw new Error('Email already registered');
      throw error;
    });
    const u = doc;
    const { accessToken, refreshToken } = signTokens(u);
...
simfinity.registerMutation(
  ...
  async (input, session) => {
    ...
    const UserModel = simfinity.getModel(simfinity.getType('user'));
    const user = await UserModel.findById(decoded.sub, { session });
