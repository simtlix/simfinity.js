...
Start with [MongoDB](./getting-started) or [PostgreSQL](./postgresql). These guides cover v3.3.0; [download the packages and compare storage semantics](./databases). Shared examples use `simfinity` for the selected runtime: the MongoDB namespace or the instance returned by `createPostgres()`.
:::
...
Explore the [Barber example app](/resources/barber) for a complete booking application with MongoDB and PostgreSQL backends and one shared frontend. The homepage follows its shop types, opening hours, and booking operations. The focused tutorials use a smaller `Serie` / `Season` domain with ES modules to explain individual API features.
