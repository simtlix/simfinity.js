...
Version 3.2.0 is prepared as an unpublished local release. From a source checkout, run `npm install` at the repository root, then add Yoga for the example without changing the lockfile: `npm install --no-save --package-lock=false graphql-yoga@^5`. For a separate local application, install the verified core and PostgreSQL archives together:

```sh
npm install /path/to/simtlix-simfinity-core-3.2.0.tgz \
  /path/to/simtlix-simfinity-postgres-3.2.0.tgz \
  graphql@^16.11.0 graphql-yoga@^5 pg@^8.16.3
```

The library supports Node.js `>=18.18.0`, GraphQL 16, and PostgreSQL 15, 16, and 18. A new application should use a maintained Node.js release.
...
Create `server.js`. The complete file registers the same GraphQL types and endpoint names used by the MongoDB runtime, builds the schema, awaits storage initialization, supplies request context to a controller, starts Yoga, and closes both the HTTP server and pool during shutdown.
...
```sh
npm install /path/to/simtlix-simfinity-mcp-3.2.0.tgz \
