...
Standalone `saveObject(typeName, args, session?, context?)` retains its existing transaction-ownership contract: without a supplied session it does not wrap hooks and nested independent entities in one transaction. Each owner's row and normalized embeddeds are nevertheless written atomically. Use a supplied transaction for atomicity across the full workflow. Reads reconstruct an owner and its embeddeds from one snapshot, preserving array order, duplicates and null items even during concurrent replacements.
...
UUID identities are a PostgreSQL storage choice for new applications. Custom code expecting 24-character ObjectIds must adapt. Empty strings retain the original materializer's absent-value behavior; a required PostgreSQL column therefore rejects an omitted/empty materialized value. PostgreSQL's typed columns also reject values MongoDB might coerce or accept. Optional stored scalars use SQL NULL for absent values; hooks should not depend on Mongoose document defaults or distinguish absence from null in native records.
