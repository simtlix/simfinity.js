...
Barber is the featured application. The homepage uses simplified `barbershop` and embedded `businessHourSlot` types, real Barber operation names, and illustrative IDs for the selected database. Link to `/resources/barber` for the complete app, including its scopes, authorization, controllers, and setup. Keep the smaller Series domain in focused tutorials and historical starter downloads.

The homepage combines a continuous animated graph, pointer and keyboard node exploration, separators drawn on entry, interactive code examples, and an explorable MCP tool list. The capabilities section keeps its introduction and a fine-line animated infinity together in a sticky left column while the capability descriptions scroll on the right. The infinity draws once and occasionally carries a softly lit point; the navigation mark stays still until hover or keyboard focus. A quick 720ms sequence moves amber and blue once around the infinity ribbon. The symbol leads with a short upward motion and tilt, followed by a left-to-right wave through the individual letters: each rises four pixels with a 24ms stagger, then settles back. The logo and text sit five pixels apart on desktop and four on mobile. Only transforms animate the lettering, so the navigation layout stays fixed. The symbol and text return to their original appearance and stay still. Leaving the link does not interrupt the lap, and entering again during a lap does not restart it. `AnimatedLogo.vue` renders both the original silhouette and the accessible wordmark through the title slot; the default `siteTitle` is disabled to avoid a duplicate label. Reduced motion keeps both static. `useHomeMotion.js` coordinates entry, bounded scroll offsets, and pointer light without changing native scrolling. On narrow screens the columns become a normal vertical flow and parallax is disabled. Animations are decorative explanations, not live runtime telemetry; `prefers-reduced-motion` removes motion. Selecting a graph node opens a hero focus view: the circuit retracts, a capability-specific SVG assembles, and masked headings, description, and code reveal in sequence. The desktop hero keeps one viewport-based height through overview, selection, and return, with content centered vertically and no bottom status strips. The capability navigation switches views directly; Back or Escape restores the original graph focus. On mobile, Back scrolls to the restored graph after its return animation. Graph nodes describe generated capabilities. The documentation uses the same type and color system with quieter reading surfaces, full-text local search, navigation, code copying, and persistent theme selection.
...
The `Documentation` GitHub Actions workflow builds on relevant pull requests and pushes to `master`, and uploads a Pages artifact. A stable release tag push or published GitHub release calls it automatically after both package registries and npm verification succeed. Version changes and merges alone do not deploy. The `github-pages` environment must allow `v*` tags as well as `master`. Documentation-only publication is also available through workflow dispatch with **deploy** selected on `master`. To enable it, select **GitHub Actions** under repository Settings → Pages → Build and deployment. Set repository variables `DOCS_BASE_PATH` and `DOCS_SITE_URL` if the public location differs from the defaults above. The repository must have Pages enabled; release automation then publishes the site without a separate manual dispatch.
...
The original MongoDB 3.0.1 starter and the earlier 3.2.0 preview archive kit remain unchanged as historical downloads. `docs/scripts/build-preview.py` rebuilds that archived snapshot only from the verified archives in its manifest and the frozen PostgreSQL server in `docs/preview/postgresql-server.js`; it is not the current release builder. The docs workflow publishes the site only. The `release.yml` coordinator creates tags, calls the reusable package workflow and then deploys this site. See the contribution guide for OIDC setup, version preparation and recovery.
