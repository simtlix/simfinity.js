...
import { castId, encodeScalar, decodeScalar } from './codecs.js';
...
export const createRecordStore = (models, database, query) => {
  ...
  const hydrate = async (storage, fields, rows, gqltype, session) => {
    ...
    for (const owned of children(storage)) {
      ...
      let ownedRows = [];
      if (ids.length) ownedRows = (await query(`SELECT * FROM ${qualified(database.schema, owned.name)} WHERE "__owner_id" = ANY ($1::uuid[])${field.list ? ' ORDER BY "__position"' : ''}`, [ids], session)).rows;
  ...
  const insertRow = async (storage, data, session) => {
    ...
    return (await query(`INSERT INTO ${qualified(database.schema, storage.name)} (${entries.map(([name]) => q(name)).join(', ')}) VALUES (${placeholders.join(', ')}) RETURNING *`, entries.map(([, value]) => value), session)).rows[0];
  };
  const persistOwned = async (storage, fields, ownerId, data, session, replace = false) => {
    for (const owned of children(storage)) {
      ...
      if (replace) await query(`DELETE FROM ${qualified(database.schema, owned.name)} WHERE "__owner_id" = $1::uuid`, [ownerId], session);
      const value = data[field.storageName];
      ...
      for (let index = 0; index < entries.length; index++) {
        ...
        const row = { __id: randomUUID(), __owner_id: ownerId, ...columnValues(owned, field.fields, entry || {}, true, entry == null) };
  ...
  const update = async (name, value, changes, session) => {
    ...
    const entries = Object.entries(data);
    let rows;
    if (entries.length) rows = (await query(`UPDATE ${qualified(database.schema, name)} SET ${entries.map(([key], index) => `${q(key)} = $${index + 2}::${storage.columns.find((column) => column.name === key).type}`).join(', ')} WHERE id = $1::uuid RETURNING *`, [id, ...entries.map(([, value]) => value)], session)).rows;
    else rows = (await query(`SELECT * FROM ${qualified(database.schema, name)} WHERE id = $1::uuid`, [id], session)).rows;
  ...
  const remove = async (name, value, session) => {
    ...
    if (!record) return null;
    await query(`DELETE FROM ${qualified(database.schema, name)} WHERE id = $1::uuid`, [record._id], session);
    return record;
  };
